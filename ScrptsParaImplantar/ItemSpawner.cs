...
public class ItemSpawner : MonoBehaviour
{
    ...
    private void SpawnarMoedaInicial()
    {
        if (moeda != null && moedaAtual == null)
        {
            SpawnarMoeda();
        }
    }
    ...
    public void SpawnarMoeda()
    {
        ...
        Vector3 posicaoTela = CalcularPosicaoTelaMoeda();
        Vector3 posicaoMundo = ConverterTelaParaMundo(posicaoTela);

        if (posicaoMundo != Vector3.zero)
        {
            moedaAtual = Instantiate(moeda, posicaoMundo, Quaternion.LookRotation(Vector3.left));

            // Configurar para spawnar nova moeda quando esta for destruída
            MoedaBehaviour moedaBehaviour = moedaAtual.GetComponent<MoedaBehaviour>();
            if (moedaBehaviour == null)
            {
                moedaBehaviour = moedaAtual.AddComponent<MoedaBehaviour>();
                moedaBehaviour.SetSpawner(this);
            }

            Debug.Log($"Moeda spawnada na posição: {posicaoMundo}");
        }
    }
    ...
    private void SpawnarPassaro()
    {
        ...
        Vector3 posicaoTela = CalcularPosicaoTelaPassaro();
        Vector3 posicaoMundo = ConverterTelaParaMundo(posicaoTela);

        if (posicaoMundo != Vector3.zero)
        {
            GameObject novoPassaro = Instantiate(passaro, posicaoMundo, Quaternion.LookRotation(Vector3.left));
            Debug.Log($"Pássaro spawnado na posição: {posicaoMundo}");
        }
    }
    ...
    private Vector3 ConverterTelaParaMundo(Vector3 posicaoTela)
    {
        ...
        float distancia = (0f - ray.origin.y) / ray.direction.y;
        return ray.origin + ray.direction * distancia;
    }

    // Chamado quando uma moeda é destruída
    public void OnMoedaDestruida()
    {
        moedaAtual = null;

        if (executando)
        {
            // Spawna nova moeda após um pequeno delay
            StartCoroutine(SpawnarNovaMoedaComDelay());
        }
    }

    private IEnumerator SpawnarNovaMoedaComDelay()
    {
